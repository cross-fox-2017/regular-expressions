...
console.log(has_ktp("234-60-1422") == true)

console.log("has_ktp returns false if it doesn't have a nomor KTP")

console.log(has_ktp("XXX-XX-1422") == false)
...
function hide_all_nomor_ktp(string) {
  if(string.match(/\d{3}-\d{2}-\d{4}/gi) ){
    //return string.replace(/[^\d{4}]/g,'X');
    //return string.replace(/\d{3}/g,'XXX');
    return string.replace(/[0-9]{3}-[0-9]{2}/g,'XXX-XX');
  }else return false;
...
function format_nomor(string) {
  if(string.match(/\d{3}.*?\d{2}.*?\d{4}/gi) ){  ///(?)   replace -
    console.log(string)
