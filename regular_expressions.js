...
function format_nomor(string) {
    let regex1 = /[.]/g;
    let regex2 = /(\d{3})(\d{2})(\d{4})/g;
    let regex3 = /(\d{3}).(\d{2}).(\d{4})/g;
    let num1 = string.match(regex2);
    let num2 = string.match(regex3);

    if(regex2.test(num1) === true){
        return `${num1.join('').replace(regex2, "$1-$2-$3")}, ${num2.join(', ').replace(regex1, '-')}`;
    }
    return typeof string;
}
...
let strg = "please confirm your identity: 44211422";
console.log(format_nomor(strg)) // string
