...
function has_ktp(string) {
	let regexHasKtp = /\d{3}-\d{2}-\d{4}/
	return regexHasKtp.test(string)
}
...
function grab_ktp(string) {
	let regexGrabKtp = /\d{3}-\d{2}-\d{4}/
	return regexGrabKtp.exec(string) 
}
...
function grab_all_nomor_ktp(string) {
	let regexAllKtp = string.match(/\d{3}-\d{2}-\d{4}/g)
	if (regexAllKtp !== null) {
		return regexAllKtp
	} else {
		return []
...
function hide_all_nomor_ktp(string) {
	let regexHideAll = string.replace(/\d{3}-\d{2}/g, "XXX-XX")
	return regexHideAll

	let regexString = /\X{3}-\X{2}/
	if (regexString.test(string) === true) {
		return string
	}
}
...
function format_nomor(string) {
	let regexFormat1 = /(\d{3})\W*(\d{2})\W*(\d{4})/gi
	return string.replace(regexFormat1, "$1-$2-$3")
}
