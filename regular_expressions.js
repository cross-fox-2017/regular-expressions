...
function has_ktp(str) {

  let pola = /\d{3}.\d{2}.\d{4}/
  return pola.test(str)
...
function grab_ktp(str) {
  let pola = /\d{3}.\d{2}.\d{4}/;
  return str.match(pola)
}
...
function grab_all_nomor_ktp(str) {
  let pola = /\d{3}.\d{2}.\d{4}/g;
  let arr = []
  if (pola.test(str) == true){
    return str.match(pola)
  } else {
    return arr
  }
...
function hide_all_nomor_ktp(str) {
  let pola = /\d{3}.\d{2}/g;
  return str.replace(pola,'XXX-XX')
}
...
var str = "please confirm your identity: XXX-XX-1422"
console.log(hide_all_nomor_ktp(str) == str)


// Ensure all of the Social Security numbers use dashes for delimiters.
// Example: 480.01.4430 and 480014430 would both be 480-01-4430.
function format_nomor(str) {
  let pola = /(\d{3})\W*(\d{2})\W*(\d{4})/g;
  return str.replace(pola, '$1-$2-$3');
}
...
str = "please confirm your identity: 44211422"
console.log(format_nomor(str) == str)
